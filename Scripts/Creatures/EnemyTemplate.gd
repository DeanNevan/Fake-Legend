...
func ai_state_alert():
	sight_line.force_raycast_update()
	#print(sight_line.is_colliding())
	if !sight_line.is_colliding() and sight_line.enabled == true:
		ai_state = "noticing"
...
func ai_state_catch():
	if sight_line.is_colliding() or !sight_line.enabled:
...
func ai_state_search():
	if !sight_line.is_colliding() and sight_line.enabled:
		ai_state = "noticing"
		return
...
func ai_move(direction, max_speed):
	move_speed += clamp((strength - total_weight) / 2.0, 2, 15)
